...
const ZODIAC_INFO: Record<string, { element: string; modality: string; ruler: string }> = {
...
interface House {
  ...
  Sign: { key: string; label: string };
  ChartPosition: {
    StartPosition: { Ecliptic: { DecimalDegrees: number; ArcDegreesFormatted30: string } };
...
interface HoroscopeResult {
  CelestialBodies: { all: CelestialBody[] };
  CelestialPoints: { all: CelestialBody[] };
  Aspects: { all: Aspect[] };
  Houses: House[];
  Angles: AnglesCollection;
}

function formatCelestialBody(body: CelestialBody): string {
...
function formatHouse(house: House): string {
...
server.tool(
  "get_birth_chart",
  "Calculate a complete natal/birth chart for a person based on their birth date, time, and location. Returns planetary positions, house placements, aspects, and key points like Ascendant and Midheaven.",
  {
  ...
  async ({ year, month, day, hour, minute, latitude, longitude, houseSystem, zodiac }) => {
    try {
      ...
      // Calculate horoscope
      const horoscope: HoroscopeResult = new Horoscope({
        origin,
        houseSystem,
        zodiac,
      ...
      const majorAspects = aspects
        .filter((a: Aspect) => ["conjunction", "opposition", "trine", "square", "sextile"].includes(a.aspectKey?.toLowerCase() || ""))
      ...
      // Get key angles
      const ascendant = horoscope.Angles?.ascendant || horoscope.Angles?.all?.find((a: Angle) => a.key === "ascendant");
      const midheaven = horoscope.Angles?.midheaven || horoscope.Angles?.all?.find((a: Angle) => a.key === "midheaven");
      ...
      const sun = celestialBodies.find((b: CelestialBody) => b.key === "sun");
      const moon = celestialBodies.find((b: CelestialBody) => b.key === "moon");
...
server.tool(
  "get_zodiac_info",
  "Get detailed information about a specific zodiac sign including its element, modality, ruling planet, and characteristics.",
  {
    sign: z
      .enum([
        "aries",
...
server.tool(
  "get_aspect_info",
  "Get detailed information about astrological aspects (angular relationships between planets).",
  {
    aspect: z
  ...
  async ({ aspect }) => {
    ...
    const aspectDetails: Record<string, { angle: number; type: string; description: string }> = {
      conjunction: {
        angle: 0,
        type: "Major",
        description:
          "A conjunction occurs when two planets are at the same degree. This is the most powerful aspect, representing a fusion of energies. The planets involved blend their influences, which can be harmonious or challenging depending on the planets involved.",
      },
      opposition: {
        angle: 180,
        type: "Major",
        description:
          "An opposition occurs when planets are directly across from each other. This aspect represents awareness through contrast, creating tension that can lead to greater understanding. It often manifests as external challenges or relationship dynamics.",
      },
      trine: {
        angle: 120,
        type: "Major",
        description:
          "A trine is considered the most harmonious aspect. Planets in trine support each other naturally, creating flow and ease. This aspect indicates natural talents and areas where things come easily, though it can sometimes lead to complacency.",
      },
      square: {
        angle: 90,
        type: "Major",
        description:
          "A square creates dynamic tension between planets. This challenging aspect represents obstacles and friction that ultimately promote growth. Squares are catalysts for action and change, pushing us to overcome limitations.",
      },
      sextile: {
        angle: 60,
        type: "Major",
        description:
          "A sextile represents opportunity and cooperation between planets. Unlike the trine, which comes naturally, sextiles require some effort to activate. They indicate areas where we can develop talents through conscious effort.",
      },
      quincunx: {
        angle: 150,
        type: "Minor",
        description:
          "Also called an inconjunct, this aspect creates an awkward energy between planets that don't naturally understand each other. It requires continuous adjustment and can indicate areas of life that need ongoing attention and adaptation.",
      },
      quintile: {
        angle: 72,
        type: "Minor",
        description:
          "A quintile indicates creative or spiritual gifts. This aspect is associated with special talents, artistic abilities, and the capacity to create something unique. It represents potential for mastery in specific areas.",
      },
      semisextile: {
        angle: 30,
        type: "Minor",
        description:
          "A semisextile connects adjacent signs, creating a subtle but persistent connection. This aspect indicates areas of minor growth and development, often working in the background of consciousness.",
      },
      semisquare: {
        angle: 45,
        type: "Minor",
        description:
          "A semisquare creates minor friction and irritation. It represents small challenges and adjustments needed, often manifesting as internal restlessness or minor external obstacles that require attention.",
      },
      sesquiquadrate: {
        angle: 135,
        type: "Minor",
        description:
          "Also called a sesquisquare, this aspect creates agitation and the need for adjustment. It's similar to a square but less intense, representing ongoing minor tensions that push for resolution.",
      },
    };
